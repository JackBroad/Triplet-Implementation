...
expTime0 = fullBoxMat[0,2]
for j in range (0,iMax+1):
  fullBoxMat[j,0]=fullT0/fullBoxMat[j,0]
  fullBoxMat[j,2]=expTime0/fullBoxMat[j,2]
  atomMoveMat[j,0]=moveT0/atomMoveMat[j,0]

fullT0 = int(fullT0)
moveT0 = int(moveT0)

nProcsExpected = np.zeros(len(nProcs))
for l in range (0,len(nProcs)):
  nProcsExpected[l] = (nProcs[l]*0.88433)+0.50908 # Line of best fit from bestFitParams.py
...
ax2.plot(nProcs,nProcsExpected,c="r",label="Expected")
mp.grid()
mp.xlabel(r"$N_{p}}$")
mp.ylabel(r"t$_1$ / t$_{N_{p}}$")
mp.legend(loc="upper left")
mp.savefig(dirString+'/'+'expectedAtomMoveSpeedUpSharedMem.pdf',bbox_inches = "tight")

figgg=mp.figure()
ax3=figgg.add_subplot(111)
ax3.scatter(nProcs,fullBoxMat[:,2],s=75,c="g",marker="o",label=r"t$_1$ = "+str(expTime0)+" s")
mp.grid()
mp.xlabel(r"$N_{p}}$")
mp.ylabel(r"t$_1$ / t$_{N_{p}}$")
mp.legend(loc="upper left")
mp.savefig(dirString+'/'+'expSpeedUpFullBoxSharedMem.pdf',bbox_inches = "tight")
